...
class TestableSychroController : public synchro_drive_controller::SychroController
{
public:
  using SychroController::SychroController;
...
class TestSychroController : public ::testing::Test
{
protected:
  ...
  void SetUp() override
  {
    controller_ = std::make_unique<TestableSychroController>();
  ...
  const std::string controller_name = "test_synchro_drive_controller";
  std::unique_ptr<TestableSychroController> controller_;
...
TEST_F(TestSychroController, configure_fails_without_parameters)
{
...
TEST_F(TestSychroController, configure_fails_if_only_traction_or_steering_side_defined)
{
  const auto ret = controller_->init(controller_name);
  ASSERT_EQ(ret, controller_interface::return_type::OK);
...
TEST_F(TestSychroController, configure_succeeds_when_joints_are_specified)
{
  const auto ret = controller_->init(controller_name);
  ASSERT_EQ(ret, controller_interface::return_type::OK);
...
TEST_F(TestSychroController, activate_fails_without_resources_assigned)
{
  const auto ret = controller_->init(controller_name);
  ASSERT_EQ(ret, controller_interface::return_type::OK);
...
TEST_F(TestSychroController, activate_succeeds_with_resources_assigned)
{
  const auto ret = controller_->init(controller_name);
  ASSERT_EQ(ret, controller_interface::return_type::OK);
...
TEST_F(TestSychroController, cleanup)
{
  const auto ret = controller_->init(controller_name);
  ASSERT_EQ(ret, controller_interface::return_type::OK);
...
TEST_F(TestSychroController, correct_initialization_using_parameters)
{
  const auto ret = controller_->init(controller_name);
  ASSERT_EQ(ret, controller_interface::return_type::OK);
